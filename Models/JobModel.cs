...
namespace Invoqs.Models
{
    ...
    public class JobModel
    {
        ...
        {
            var dateStr = JobDate.ToString("dd/MM/yyyy");
            var description = $"{TypeDisplayName} - {Address} ({dateStr})";
            
            // Add job title if it's different from type
            if (!string.IsNullOrEmpty(Title) && Title != TypeDisplayName)
            {
                description = $"{Title} - {TypeDisplayName} - {Address} ({dateStr})";
            }
    ...
    public class AddressJobGroup
    {
        ...
        public decimal TotalRevenue => Jobs.Sum(j => j.Price);
        public DateTime? EarliestJobDate => Jobs.Min(j => j.JobDate);
        public DateTime? LatestJobDate => Jobs.Max(j => j.JobDate);

        // Invoice-related properties for address groups
